...
1. In Workchat, go to **System Console > Plugins > Nagios**.
...
5. Switch to the machine where Nagios is running:
  - Download the latest stable version of the watcher from the [releases page](https://gitlab.com/w1572/workchat-plugin-nagios/releases)
...
```shell script
sudo bash -c 'cat << EOF > /etc/systemd/system/workchat-plugin-nagios-watcher.service
[Unit]
...
[Service]
Restart=on-failure
ExecStart=/usr/local/bin/watcher -dir /nagios/configuration/files/directory -url https://workchat.server.address/plugins/nagios -token TheTokenFromStep1
...
systemctl enable workchat-plugin-nagios-watcher.service
systemctl start  workchat-plugin-nagios-watcher.service
```
